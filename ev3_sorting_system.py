...
def message_callback(topic, msg):
    global stop_program, settings_dict, start, start_time, end_time, button_ctrl, rolling, belt, activate, sync, zone_occupied, zone_available
...
client = MQTTClient(client_id, broker_address, broker_port, username, password)
...
def rolling_belt():
    ...
    print('BELT STARTED')
    publish(topic_output, "BELT STARTED")
    client.check_msg()
    settings_dict['preset']= 'auto'
    settings_dict['pickuppoint_auto'] = 1
    settings_dict['z1_altitude'] = 90
    belt.stop()
    publish(topic_output, "MOVING TO PICKUP ZONE")
    start_on_pickup_zone(settings_dict)
    while rolling:
        client.check_msg()
        belt.run(35)
        ...
        if red > 3 or green > 3 or blue > 3:
            ...
            publish(topic_output, 'COLOR: ' + itemcolor)
            publish(topic_output, "PICKING UP ITEM")
            opengripper()
            elbow.run_target(150, 65)
            closegripper()
            elbow.run_target(150, 90)
            publish(topic_output, "SORTING ITEM")
            sort_item(itemcolor, settings_dict)
            dropoff()
            publish(topic_output, "ITEM SORTED")
            elbowup()
            elbow.run_target(150, 90)
            start_on_pickup_zone(settings_dict)
    ...
    if not activate:
        while button_ctrl:
            ...
            if Button.CENTER in ev3.buttons.pressed():
                ...
                button_ctrl = False
        sensor_ctrl = False
        publish(topic_output, "SENSOR POSITION CALIBRATED")
        return value
...
def calculate_average_speed(item_count, current_time, last_time):
    time_elapsed = current_time - last_time
    time_elapsed_hours = time_elapsed / 3600

    # Calculate average speed in items per hour
    if time_elapsed_hours > 0:
        average_speed = item_count / time_elapsed_hours
    else:
        average_speed = 0  # To avoid division by zero if no items processed yet

    publish(topic_avg, str(average_speed))

def main_loop():
    item_count=0
    last_time = time.time() + 2*3600
    # Main loop
    while True:
        if stop_program:
            sys.exit()
            ...
            while continue_main_loop:
                ...
                itemcolor = ""
                if sync == True:
                    publish(topic_output, "SYNC ON")
                    if zone_occupied:
                        publish(topic_output, "ZONE OCCUPIED!")
                        while True:
                            base.stop()
                            elbow.stop()
                            gripper.stop()
                            client.check_msg()
                            if zone_available:
                                publish(topic_output, "ZONE AVAILABLE")
                                break
                    elif zone_available and base.angle() <= 60:
                        while True:
                            publish(topic_com, "STOP")
                            client.check_msg()
                            ...
                            red, green, blue = read_rgb()
                            itemcolor = rgb_to_color(red, green, blue)
                            publish(topic_output, itemcolor)
                            showinfo(itemcolor)
                            client.check_msg()
                            if stop_program or not continue_main_loop: break
                ...
                if itemcolor.lower() is not 'other':
                    ...
                    elbowup()
                    item_count += 1
                    calculate_average_speed(item_count, current_time, last_time)
                client.check_msg()
                if stop_program or not continue_main_loop: break
